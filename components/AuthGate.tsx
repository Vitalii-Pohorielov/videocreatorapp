...
export function AuthGate({
  ...
  description = "Use Google sign-in to open the editor, edit scenes, and manage your saved projects.",
  requirePremium = false,
  premiumTitle = "Premium required",
  premiumDescription = "Your account needs the premium flag enabled before you can create or edit videos.",
}: AuthGateProps) {
  const { isLoading, user, error } = useAuthSession();
  const premiumState = usePremiumStatus();

  if (isLoading || (requirePremium && user && premiumState.isLoading)) {
    return (
      <main className="flex min-h-[calc(100vh-72px)] items-center justify-center px-6 text-slate-100">
        <div className="rounded-[28px] border border-white/10 bg-slate-950/70 px-8 py-10 text-center shadow-[0_16px_40px_rgba(2,6,23,0.35)]">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-white">{requirePremium ? "Checking your access" : "Checking your session"}</h1>
        </div>
      </main>
    );
  }
  ...
  if (requirePremium && !premiumState.isPremium) {
    return (
      <main className="flex min-h-[calc(100vh-72px)] items-center justify-center px-6 text-slate-100">
        <div className="w-full max-w-2xl rounded-[32px] border border-white/10 bg-slate-950/75 px-8 py-10 shadow-[0_24px_80px_rgba(2,6,23,0.4)] backdrop-blur">
          <p className="text-xs uppercase tracking-[0.28em] text-sky-300">Premium Access</p>
          <h1 className="mt-4 text-4xl font-semibold tracking-[-0.05em] text-white">{premiumTitle}</h1>
          <p className="mt-4 text-base leading-7 text-slate-300">{premiumDescription}</p>
          {premiumState.error ? <p className="mt-4 text-sm text-rose-300">{premiumState.error}</p> : null}
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href="/projects"
