...
alter table public.banner_projects
add column if not exists deleted boolean not null default false;
...
create or replace function public.set_banner_projects_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists set_banner_projects_updated_at on public.banner_projects;

create trigger set_banner_projects_updated_at
before update on public.banner_projects
for each row
execute function public.set_banner_projects_updated_at();

alter table public.banner_projects enable row level security;

drop policy if exists "Users can read own banner projects" on public.banner_projects;
drop policy if exists "Users can insert own banner projects" on public.banner_projects;
drop policy if exists "Users can update own banner projects" on public.banner_projects;
drop policy if exists "Users can delete own banner projects" on public.banner_projects;

create policy "Users can read own banner projects"
on public.banner_projects
for select
to authenticated
using (auth.uid() = user_id);

create policy "Users can insert own banner projects"
on public.banner_projects
for insert
to authenticated
with check (auth.uid() = user_id);

create policy "Users can update own banner projects"
on public.banner_projects
for update
to authenticated
using (auth.uid() = user_id)
...
create policy "Users can delete own banner projects"
on public.banner_projects
for delete
to authenticated
using (auth.uid() = user_id);
